...
| 백엔드 | Node.js (JavaScript), Express, JWT (Access 15분 / Refresh 7일) |
...
**완료 조건**
- [x] `backend/src/db/migrations/`에 5개 파일 모두 존재하고 파일명이 요구사항 형식과 일치한다
- [x] 001→005 순서로 빈 DB에 실행 시 오류 없이 완료되며 테이블 4개·인덱스 8개·트리거 2개가 생성된다
- [x] `005_seed_default_categories.sql`을 2회 이상 반복 실행해도 데이터 중복이 발생하지 않는다
- [x] schema.sql 기준 모든 FK·CHECK·UNIQUE 부분 인덱스가 분리 파일에 동일하게 포함된다
...
`backend/src/db/pool.js` 파일을 작성하여 애플리케이션 전역에서 단일 Pool 인스턴스를 공유한다.
...
**완료 조건**
- [x] `export const pool`과 `export async function closePool()` 모두 export된다
- [x] Pool 설정 4개 항목(connectionString, max, idleTimeoutMillis, connectionTimeoutMillis)이 요구사항 값과 정확히 일치한다
- [x] `pool.on('error', ...)` 핸들러가 등록되어 오류 발생 시 프로세스가 비정상 종료되지 않는다
- [x] `closePool()` 호출 시 `pool.end()`가 실행되어 모든 커넥션이 정상 해제된다
...
`backend/src/config/env.js` 작성. 애플리케이션 시작 시 필수 환경변수 존재 여부·형식을 검증하고 실패 시 즉시 `process.exit(1)`.
...
**완료 조건**
- [x] `DATABASE_URL`이 누락된 상태에서 실행 시 변수명을 포함한 오류 메시지와 함께 프로세스가 종료된다
- [x] `DATABASE_URL`에 허용 프로토콜 외 값이 입력되면 형식 오류로 프로세스가 종료된다
- [x] 검증 통과 시 반환되는 config 객체의 모든 필드가 올바른 값으로 설정된다
- [x] `backend/.env.example` 파일에 DB 관련 환경변수 키가 모두 포함되어 있다
...
**완료 조건**
- [x] `.env.development`와 `.env.test`가 각각 다른 DB명(`todolist_dev` vs `todolist_test`)을 사용한다
- [x] `backend/.gitignore`에 `.env.development`, `.env.test`, `.env.production`이 명시되어 있다
- [x] `database/create_databases.sql` 실행 시 두 DB가 생성된다
- [x] `.env.example`에 실제 자격정보가 없고 플레이스홀더로만 작성되어 있다
...
**완료 조건**
- [x] `checkDatabaseConnection()`이 DB 응답 불가 시 예외 없이 `false`를 반환한다
- [x] `getDatabaseStatus()`의 반환 객체가 5개 필드(connected, latencyMs, poolTotal, poolIdle, poolWaiting)를 모두 포함한다
- [x] `runMigrations()` 2회 실행 시 이미 적용된 마이그레이션을 건너뛰어 중복 오류가 발생하지 않는다
- [x] 모든 함수에서 Pool 커넥션이 `finally` 블록 내 `release()`로 반환된다
...
`backend/src/server.js`에 DB 연결 검증·마이그레이션 실행·Graceful Shutdown을 통합한다.
...
**완료 조건**
- [x] 잘못된 `DATABASE_URL`로 서버 시작 시 연결 실패 메시지와 함께 exit code 1로 종료된다
- [x] 정상 시작 시 포트·`NODE_ENV`·DB 연결 성공 메시지가 로그에 출력된다
- [x] SIGTERM 수신 시 `closePool()`이 호출되고 프로세스가 정상 종료된다
- [x] `NODE_ENV=production`에서는 `runMigrations()`가 실행되지 않는다
...
`backend/tests/helpers/dbSetup.js` 작성. Mock DB 사용 금지 — `.env.test`의 `todolist_test` DB 대상.
...
**완료 조건**
- [x] `initTestDatabase()` 호출 시 `todolist_test` DB에 테이블 4개·기본 카테고리 3건이 준비된다
- [x] `clearAllTables()` 호출 후 사용자 데이터 행이 0개이고 기본 카테고리 3건은 유지된다
- [x] `seedTestUser()`가 반환한 `userId`로 users 테이블 조회가 가능하다
- [x] `getTestPool()`이 `todolist_dev` DB를 가리키는 URL로 호출되면 오류를 던진다
- [x] 모든 DB 쿼리에서 커넥션이 `finally` 블록 내 `release()`로 반환된다
...
- `package.json`: express, pg, bcrypt, jsonwebtoken, cookie-parser, cors, express-rate-limit, zod, dotenv / jest, supertest devDependencies. scripts: `dev`, `start`, `test`, `migrate`
- `.eslintrc.json`: eslint:recommended, no-unused-vars
- `.prettierrc`: singleQuote true, trailingComma all, printWidth 100
- `jest.config.js`: unit/integration 프로젝트 분리, coverageThreshold 80%
- `.env.example`: DATABASE_URL, JWT_ACCESS_SECRET, JWT_REFRESH_SECRET, PORT, NODE_ENV, BCRYPT_SALT_ROUNDS 항목 명시

**완료 조건**
- [x] `npm install` 성공, `npm audit` 취약점 없음
- [x] ESLint, Prettier 실행 시 경고·오류 없음
- [x] `jest --listTests`로 unit/integration 프로젝트가 분리 인식됨
...
- Pool 설정값(max: 20, idleTimeoutMillis: 30000)이 env.js에서 주입되며 하드코딩 없음
...
**완료 조건**
- [x] `npm run migrate` 실행 시 PostgreSQL에 4개 테이블·인덱스가 정상 생성된다
- [x] `withTransaction` 헬퍼가 콜백 예외 발생 시 ROLLBACK을 보장한다
- [x] Pool 설정값이 env.js에서 주입되며 코드에 하드코딩이 없다
- [x] 마이그레이션 재실행 시 이미 적용된 파일은 건너뛴다
...
- `src/utils/jwt.js`: `signAccessToken` (15분), `signRefreshToken` (7일), `verifyAccessToken`, `verifyRefreshToken`
- `src/utils/hash.js`: bcrypt `hashPassword` / `comparePassword` (salt rounds env 주입)
- `src/utils/tokenHash.js`: Refresh Token SHA-256 단방향 해싱 (`crypto` 내장 모듈)
- `src/utils/errors.js`: `AppError(base)` → `UnauthorizedError(401)`, `ForbiddenError(403)`, `NotFoundError(404)`, `ConflictError(409)`, `ValidationError(422)`

**완료 조건**
- [x] `verifyAccessToken`이 만료·변조 토큰에 대해 `UnauthorizedError`를 throw한다
- [x] `hashPassword` → `comparePassword` 왕복 검증이 정상 동작한다
- [x] `hashToken`이 동일 입력에 대해 항상 동일한 SHA-256 hex 문자열을 반환한다
- [x] 모든 커스텀 에러가 `AppError instanceof` 체크를 통과한다
...
- `authenticate.js`: Authorization 헤더 Bearer 토큰 → `verifyAccessToken` → `req.user` 세팅. 토큰 없음/만료 시 `UnauthorizedError`
- `validate.js`: `validate(schema)` 팩토리 — req.body zod 파싱 실패 시 `ValidationError(422)`
- `rateLimiter.js`: `express-rate-limit` 기반, windowMs: 60000, max: 10. `/api/auth/login`·`/api/auth/signup`에만 적용
- `errorHandler.js`: `AppError` → `{ success: false, error: { code, message } }` JSON 응답. 기타 오류 → 500. production에서 스택트레이스 미노출
- `src/app.js`: cors (CORS_ORIGIN env), json 파서, cookie-parser, 라우터 마운트, errorHandler 등록

**완료 조건**
- [x] 유효하지 않은 JWT로 보호 엔드포인트 접근 시 401 JSON 응답이 반환된다
- [x] zod 스키마 위반 요청 시 422 응답과 필드별 오류 상세가 반환된다
- [x] `/api/auth/login`에 분당 11회 요청 시 11번째에 429 응답이 반환된다
- [x] 처리되지 않은 예외 발생 시 500 응답을 반환하고 스택트레이스가 응답 바디에 포함되지 않는다(production)
...
- `user.repository.js`: `createUser`, `findByEmail`, `findById`, `updateUser`(동적 SET, updated_at 갱신), `deleteUser(PoolClient)` (트랜잭션 참여)
- `refreshToken.repository.js`: `createToken`, `findByTokenHash`, `deleteByTokenHash` (Token Rotation 시 기존 삭제), `deleteByUserId(PoolClient)` (회원탈퇴 트랜잭션용), `deleteExpiredTokens` (배치용)

**완료 조건**
- [x] `createUser` 후 `findByEmail`로 동일 레코드를 조회할 수 있다
- [x] `findByEmail`이 존재하지 않는 이메일에 대해 null을 반환한다
- [x] `updateUser`가 전달된 필드만 UPDATE하고 나머지 컬럼을 보존한다
- [x] `deleteUser`가 PoolClient를 받아 외부 트랜잭션에 참여한다(auto-commit 없음)
...
- `auth.controller.js`: 쿠키 정책 — `HttpOnly + Secure + SameSite=Strict`, maxAge 7일
- `auth.route.js`: POST /signup, /login (authRateLimiter 적용), /logout, /refresh
- Zod 스키마: signup(email, password min:8 영문+숫자, name), login(email, password)

**완료 조건**
- [x] 동일 이메일로 두 번 signup 시 409 ConflictError가 반환된다
- [x] login 성공 시 응답 바디에 accessToken, Set-Cookie에 refreshToken(HttpOnly)이 포함된다
- [x] refresh 호출 시 기존 refreshToken DB 레코드가 삭제되고 신규 레코드가 생성된다(Token Rotation)
- [x] 만료된 refreshToken으로 /refresh 호출 시 401이 반환된다
- [x] /login·/signup에 분당 10회 초과 요청 시 429가 반환된다
...
- `user.controller.js`: authenticate 미들웨어 통과 후 req.user.sub 활용
- `user.route.js`: 전체 라우트에 authenticate 적용. GET/PATCH/DELETE /api/users/me

**완료 조건**
- [x] GET /api/users/me 응답에 password_hash 필드가 포함되지 않는다
- [x] 비밀번호 변경 시 currentPassword 불일치하면 401이 반환된다
- [x] DELETE /api/users/me 실행 시 단일 트랜잭션으로 4개 테이블 데이터가 모두 삭제된다
- [x] 트랜잭션 중 오류 발생 시 ROLLBACK되어 부분 삭제가 없다
- [x] authenticate 미들웨어 없이 접근 시 401이 반환된다
...
- `category.repository.js`: `findAllByUserId`(기본 카테고리 포함), `findById`, `create`, `findDefaultByUserId`, `deleteById(PoolClient)`, `deleteByUserId(PoolClient)`, `reassignTodos(PoolClient, fromId, toId, userId)`
- `category.service.js`:
...
- `category.route.js`: 전체 authenticate 적용. GET/POST/DELETE /api/categories

**완료 조건**
- [x] GET /api/categories 응답에 is_default: true인 '일반' 카테고리가 항상 포함된다
- [x] 기본 카테고리 id로 DELETE 요청 시 403이 반환된다(BR-06)
- [x] 카테고리 삭제 시 소속 할일의 category_id가 단일 트랜잭션으로 기본 카테고리 id로 변경된다(BR-07)
- [x] 타 사용자 카테고리 삭제 시도 시 403이 반환된다(BR-03)
- [x] 동일 사용자 내 카테고리 이름 중복 생성 시 409가 반환된다
...
- `todo.repository.js`: `findAll(userId, filter)`(동적 WHERE — categoryId, isCompleted, overdue), `findById`, `create`, `update`(동적 SET), `deleteById`, `deleteByUserId(PoolClient)`, `reassignCategory(PoolClient, fromId, toId, userId)`
- `todo.service.js`:
...
- `todo.route.js`: 전체 authenticate 적용. GET/POST/PATCH/DELETE /api/todos
- Zod 쿼리 파라미터: `categoryId`(optional UUID), `isCompleted`(optional boolean), `overdue`(optional boolean)

**완료 조건**
- [x] GET /api/todos?isCompleted=false&overdue=true 요청 시 기간 만료된 미완료 할일만 반환된다(BR-08)
- [x] 타 사용자 소유 category_id로 할일 생성 시 403·404가 반환된다(BR-04)
- [x] due_date < start_date인 요청 시 422가 반환된다(BR-05)
- [x] 타 사용자 할일 수정·삭제 시도 시 403이 반환된다(BR-03)
...
- `tests/unit/auth.service.test.js`: 이메일 중복 ConflictError, bcrypt 호출, Token Rotation 동작
- `tests/unit/category.service.test.js`: 기본 카테고리 삭제 ForbiddenError(BR-06), 소유권 없음 ForbiddenError(BR-03), withTransaction 내 재배정+삭제 순서(BR-07)
- `tests/unit/todo.service.test.js`: 카테고리 미소유 ForbiddenError(BR-04), 날짜 역전 ValidationError(BR-05), 소유권 확인(BR-03)

**완료 조건**
- [x] `npm test -- --testPathPattern=unit` 전체 통과
- [x] 각 Service 파일 라인 커버리지 ≥ 85%
- [x] Repository 실제 구현에 의존하지 않음(jest.mock 격리 확인)
- [x] 비즈니스 규칙 BR-01~BR-08 관련 분기가 최소 1개 이상 테스트 케이스로 커버된다
- [x] 테스트 실행 시간 30초 이내(DB 연결 없음)
...
- `tests/integration/setup.js`: 테스트 DB 연결 설정, 각 테스트 전 TRUNCATE(CASCADE), 완료 후 pool.end()
- `tests/integration/auth.test.js`: signup(201, 중복 409, 유효성 422), login(200, 잘못된 비밀번호 401), refresh(쿠키 갱신), logout
- `tests/integration/user.test.js`: getMe(password_hash 미포함), updateMe(비밀번호 불일치 401), deleteMe(204, 이후 login 불가)
- `tests/integration/category.test.js`: 기본 카테고리 포함 조회, 중복명 409, 기본 카테고리 삭제 거부, 소속 할일 재배정 DB 직접 확인(BR-07), 타 사용자 403
- `tests/integration/todo.test.js`: 필터 정확성(categoryId, isCompleted, overdue), 잘못된 categoryId 403, 날짜 역전 422, 타 사용자 403

**완료 조건**
- [x] `npm test -- --testPathPattern=integration` 전체 통과(실제 PostgreSQL 연결 필수)
- [x] 각 테스트 간 데이터 격리 확인(TRUNCATE 또는 트랜잭션 롤백)
- [x] 전체 API 엔드포인트 14개 모두 최소 1개 이상 통합 테스트 케이스가 존재한다
- [x] BR-03(403), BR-06(기본 카테고리), BR-07(재배정) 시나리오가 DB 레코드 직접 조회로 검증된다
- [x] unit+integration 합산 전체 커버리지 ≥ 80%
...
**완료 조건**
- [x] `npm run dev` 실행 시 브라우저에서 Vite 기본 화면이 정상 렌더링된다
- [x] `npm run type-check`(tsc --noEmit) 오류 없이 통과한다
- [x] `@/` path alias로 모듈 import 시 TypeScript 타입 추론이 정상 동작한다
- [x] `npm run lint` 실행 시 ESLint 오류 0건이다
...
**완료 조건**
- [x] 모든 타입 파일에서 `any` 사용 없이 strict 타입 정의가 완료된다
- [x] `Todo.dueDate`가 `string | null`(ISO 8601 UTC)으로 정의되어 dayjs 변환에 활용할 수 있다
- [x] `Category.isDefault`가 `boolean`으로 정의되어 삭제 버튼 비활성화 로직에 활용할 수 있다
- [x] `tsc --noEmit` 오류 없이 통과한다
...
**완료 조건**
- [x] 인증된 요청에 `Authorization` 헤더가 자동 주입된다(vitest 단위 테스트로 확인)
- [x] Access Token 만료 시 자동으로 refresh 후 원본 요청을 재시도한다(MSW mock 활용)
- [x] 동시 3개 요청이 401 응답을 받아도 refresh API가 1회만 호출된다
- [x] Refresh Token 만료 시 Zustand store 초기화 및 `/login` 리다이렉트가 동작한다
...
**완료 조건**
- [x] `auth.store`의 `accessToken`이 메모리에만 존재하고 브라우저 스토리지에 persist되지 않음을 테스트로 확인한다
- [x] `clearAuth()` 호출 시 `accessToken`·`user` 모두 null로 초기화된다
- [x] `showToast` 호출 후 3000ms 이내에 해당 Toast가 `toastQueue`에서 제거된다(vitest fake timers)
...
**완료 조건**
- [x] `Modal`에서 ESC 키 누르면 닫히고, 열렸을 때 포커스가 모달 내부로 이동한다
- [x] `Input`에 `error` prop 전달 시 `aria-invalid="true"` 및 error 메시지가 DOM에 렌더링된다
- [x] `Toast`가 `ui.store`의 `toastQueue` 변경에 반응하여 자동 렌더링·소멸된다
- [x] `Button` loading 상태에서 `disabled` 처리되어 중복 클릭이 방지된다
- [x] 360px 뷰포트에서 `Layout`의 네비게이션이 가로 스크롤 없이 정상 표시된다 (하단 탭 네비게이션, md:hidden 적용)
...
**완료 조건**
- [x] 비인증 상태에서 `/` 접근 시 `/login`으로 redirect된다
- [x] 인증 상태에서 `/login` 접근 시 `/`로 redirect된다
- [x] 페이지 새로고침 후 `useAuthInitializer`가 `/api/users/me`를 호출하여 인증 상태가 복구된다
- [x] 초기화 완료 전 전체 화면 Spinner가 표시된다
...
**완료 조건**
- [x] 잘못된 이메일 형식 입력 시 즉시(onBlur) 인라인 에러 메시지가 표시된다
- [x] 로그인 성공 시 `auth.store`에 `accessToken`이 저장되고 `/`로 이동한다
- [x] 이메일 중복 409 응답 시 "이미 사용 중인 이메일입니다" Toast가 표시된다
- [x] 로그아웃 후 `accessToken`이 null로 초기화되고 `/login`으로 이동한다
...
**완료 조건**
- [x] `QueryClientProvider`가 App 전체를 감싸고 있다
- [x] Query key 팩토리를 통해 invalidation 시 관련 쿼리만 정확히 무효화된다
- [x] `staleTime` 설정으로 1분 이내 재마운트 시 네트워크 요청이 발생하지 않는다
...
**완료 조건**
- [x] 기본 카테고리(`isDefault: true`)의 삭제 버튼이 disabled 상태로 렌더링된다
- [x] 카테고리 삭제 시 확인 Modal이 표시되고 취소 시 삭제가 실행되지 않는다
- [x] 카테고리 추가 성공 후 목록이 즉시 갱신된다(invalidateQueries 동작 확인)
- [x] 카테고리 삭제 후 할일 목록 쿼리도 무효화되어 재조회된다
...
**완료 조건**
- [x] 완료 체크박스 클릭 시 서버 응답 전에 UI가 즉시 반영되고, 실패 시 롤백된다(Optimistic Update)
- [x] `isOverdue()` 함수가 클라이언트 로컬 타임존 기준으로 기간만료를 올바르게 판단한다(dayjs unit test)
- [x] 카테고리 필터 변경 시 URL 쿼리스트링이 업데이트되고 목록이 재조회된다
- [x] 삭제 확인 Modal에서 취소 시 삭제가 실행되지 않는다
- [x] 완료 여부·카테고리·기간만료 3가지 필터를 동시에 적용하면 조건에 맞는 항목만 표시된다
...
**완료 조건**
- [x] `dueDate`가 `startDate`보다 이전 날짜이면 "종료예정일은 시작일 이후여야 합니다" 에러가 표시된다
- [x] `categoryId` 미선택 시 폼 제출이 불가하고 에러 메시지가 표시된다(BR-04)
- [x] 수정 모드에서 기존 데이터가 폼에 사전 입력된다
- [x] 등록·수정 성공 후 할일 목록 쿼리가 무효화되고 `/`로 이동한다
...
**완료 조건**
- [x] 이름 수정 성공 시 Header에 표시된 사용자 이름이 즉시 반영된다(Zustand store 갱신)
- [x] 회원 탈퇴 플로우가 2단계(비밀번호 확인 → 최종 확인)로 진행된다
- [x] 비밀번호 확인 Modal에서 취소 시 탈퇴 프로세스가 중단된다
- [x] 탈퇴 성공 후 `accessToken`이 초기화되고 `/login`으로 이동한다
...
**완료 조건**
- [x] 360px 뷰포트에서 모든 페이지에 가로 스크롤바가 발생하지 않는다
- [x] 768px 뷰포트에서 레이아웃 전환(모바일→태블릿)이 자연스럽게 동작한다
- [x] 1280px 뷰포트에서 콘텐츠가 max-width를 초과하지 않고 중앙 정렬된다
- [x] 인터랙티브 요소의 최소 터치 타겟이 44×44px 이상이다
...
- `src/test/features/todo/TodoItem.test.tsx`: Optimistic Update·롤백, 기간만료 Badge, 삭제 확인 Modal
- `src/test/features/auth/LoginForm.test.tsx`: 유효성 오류, 성공 시 store 토큰 저장, 401 Toast
- `src/test/features/category/CategoryList.test.tsx`: 기본 카테고리 삭제 버튼 disabled, 삭제 확인 Modal
- `src/test/hooks/useAuthInitializer.test.tsx`: 초기화 시 `/api/users/me` 호출, 실패 시 `/login` navigate
- `src/test/utils/dateUtils.test.ts`: `isOverdue` 경계값 테스트(오늘, 어제, 내일, null)

**완료 조건**
- [x] `npm run test` 실행 시 전체 테스트가 통과한다
- [x] 컴포넌트 테스트 커버리지 ≥ 85%
- [x] `isOverdue` 경계값(오늘 날짜 포함) 테스트 케이스가 모두 통과한다
- [x] MSW 핸들러가 `src/test/mocks/handlers.ts`에 API별로 정리되어 재사용 가능하다
- [x] Optimistic Update 롤백 시나리오가 테스트로 검증된다
...
- `playwright.config.ts`: `baseURL: 'http://localhost:5173'`, `use.trace: 'on-first-retry'`, `globalSetup: './tests/e2e/global.setup.ts'`
- `tests/e2e/global.setup.ts`: 테스트 실행 전 2개 사용자(카테고리/할일) 사전 생성, `.auth/*.json`에 storageState 저장 (Rate Limit 대응)
...
**완료 조건**
- [x] `npx playwright test` 실행 시 전체 E2E 테스트가 통과한다
- [x] `auth.spec.ts` 회원가입→로그인→로그아웃 플로우가 10초 이내에 완료된다
- [x] 카테고리 삭제 후 할일 목록 재조회 시 해당 할일의 카테고리가 기본 카테고리로 변경됨을 확인한다
- [x] 테스트 실패 시 `test-results/`에 스크린샷·트레이스 파일이 저장된다
...
| 1.1.0 | 2026-05-15 | MinYoung | FE-14·FE-15 완료 반영 — 테스트 경로를 실제 구조(`src/test/`)로 수정, E2E globalSetup 추가, FE-15 완료 체크박스 모두 체크 |
